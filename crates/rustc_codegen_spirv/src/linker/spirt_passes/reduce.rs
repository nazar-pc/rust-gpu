...
use spirt::{
    ...
    EntityOrientedDenseMap, FuncDefBody, Node, NodeDef, NodeKind, Region, SelectionKind, Type,
    Value, Var, VarDecl, VarKind, scalar, spv, vector,
};
use std::iter;
                ...
                if let SelectionKind::Switch { case_consts } = kind {
                    if cases.len() == 3 {
                        ...
                        if case_consts.map(|ct| ct.int_as_u32()) == [Some(0), Some(1)] {
                            ...
                            let redu_target =
                                ReductionTarget::SwitchToIfElse(func_at_node.position);
            ...
            NodeDef {
                    ...
                    .zip_eq(inputs.iter().zip_eq(&body_def.outputs))
                {
                    if body_output == Value::Var(body_input_var) {
                        if let entry @ None = var_replacements.entry(body_input_var) {
